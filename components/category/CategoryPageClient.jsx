...
export default function CategoryPageClient({ slug }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const bestResp = await fetch(`${API}/api/products?categoryId=${encodeURIComponent(param)}&badge=best_seller&page=1&limit=50`);
        ...
        if (catBest.length > 0) {
          ...
          const globalResp = await fetch(`${API}/api/products?badge=best_seller&page=1&limit=50`).catch(() => null);
          const globalJson = globalResp ? await globalResp.json().catch(() => ({})) : {};
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      try {
        ...
        const selectedCategoryIds = activeFilters.expandedSubIds && activeFilters.expandedSubIds.size > 0
        ...
        if (activeFilters.minRating !== null && activeFilters.minRating !== undefined) {
  ...
  const maxBestSellingStart = Math.max(0, bestSelling.length - bestSellingPerView);
  const canSlideBestSelling = bestSelling.length > bestSellingPerView;
  const visibleBestSelling = bestSelling.slice(bestSellingStartIndex, bestSellingStartIndex + bestSellingPerView);
  ...
  return (
    <>
      ...
      <div className="mb-3 text-center">
        <h1 className="text-2xl md:text-4xl font-bold text-gray-900">{category?.name || slug}</h1>
        ...
        <>
          <div className="grid grid-cols-4 md:grid-cols-6 gap-2 md:gap-4 justify-items-center ">
            {visibleSubcategories.map((sub) => {
              ...
              return (
                <div key={sub._id} className="relative flex flex-col items-center w-full max-w-[92px] md:max-w-[110px]">
                  <Link href={`/category/${sslug}`} className="flex flex-col items-center group cursor-pointer w-full">
                    <div className="w-18 h-18 md:w-34 md:h-34 lg:w-38 lg:h-38 rounded-full bg-[#FFF5ED] border-4 border-white shadow-md flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform">
                      <Image src={(sub.images && sub.images[0] && sub.images[0].url) ? sub.images[0].url : '/assets/placeholder.svg'} alt={sub.name} width={80} height={80} className="w-24 h-24 md:w-38 md:h-38 lg:w-42 lg:h-42 object-contain" />
                    </div>
                    <div className="mt-2 text-xs md:text-sm text-center font-medium text-gray-700 hover:text-rose-600 line-clamp-2">{sub.name}</div>
                  </Link>
                </div>
              );
            })}
          ...
          {immediateChildren.length > subcategoryVisibleCount && (
            <div className="flex justify-center mb-8">
              ...
              >
                {showAllSubcategories ? 'Show less' : `+ Show all (${immediateChildren.length})`}
              </button>
            </div>
          )}
        </>
          ...
          <div className="relative pt-8 pb-3">
            {canSlideBestSelling && (
              <>
                <button
                  type="button"
                  onClick={() => setBestSellingStartIndex(i => Math.max(0, i - 1))}
                ...
                <button
                  type="button"
                  onClick={() => setBestSellingStartIndex(i => Math.min(maxBestSellingStart, i + 1))}
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-8">
        ...
        <div className="lg:hidden sticky top-16 z-30 bg-[#FFF5ED] py-2 mb-3 shadow-2xl">
          <div className="grid grid-cols-[auto_1fr_auto] gap-2 items-center rounded-lg bg-[#FCECF2] p-2">
            ...
            <SortDropdown
              value={sortOption}
              onChange={(value) => { setSortOption(value); setCurrentPage(1); }}
              className="w-full"
            />
          ...
          {!productsLoadedOnce && loadingProducts ? (
            ...
            <>
              <div className="hidden lg:flex items-center justify-between mb-3">
                <h2 className="text-xl md:text-3xl font-semibold mt-0 mb-3">All Products</h2>
                <SortDropdown value={sortOption} onChange={(value) => { setSortOption(value); setCurrentPage(1); }} />
              </div>
              ...
              {loadingProducts && (
                <div className="mt-4 text-center text-sm text-gray-500">Updating products...</div>
              )}
            </>
          ) : (
