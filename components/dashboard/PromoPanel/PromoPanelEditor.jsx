...
function ProductSearchSelect({ selectedProducts, onChange, API }) {
  ...
  const isSelected = (id) => selectedProducts.some(p => (p._id || p.id) === id);
  ...
  const remove = (id) => onChange(selectedProducts.filter(p => (p._id || p.id) !== id));

  return (
    <div>
      <label className="block text-sm font-semibold text-gray-700 mb-2">Products (shown in Popular Picks carousel)</label>
      ...
      {selectedProducts.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3 p-3 bg-gray-50 rounded-lg border">
          {selectedProducts.map((p) => {
            ...
            return (
              <div key={id} className="flex items-center gap-1.5 bg-white border rounded-lg px-2 py-1 text-xs shadow-sm">
                <Image src={img} alt={p.title} width={24} height={24} className="w-6 h-6 object-contain rounded" />
                <span className="max-w-30 truncate text-gray-800">{p.title}</span>
                <button onClick={() => remove(id)} className="text-gray-400 hover:text-red-500 ml-1 font-bold leading-none">×</button>
              </div>
            );
          })}
        </div>
        ...
        <div className="mt-1 border rounded-lg divide-y max-h-52 overflow-y-auto bg-white shadow-lg z-10 relative">
          {results.map((p) => {
            ...
            return (
              ...
              >
                <Image src={img} alt={p.title} width={32} height={32} className="w-8 h-8 object-contain rounded border shrink-0" />
                <span className="flex-1 truncate text-gray-800">{p.title}</span>
                {selected && <span className="text-blue-600 font-bold text-xs shrink-0">✓ Added</span>}
              </button>
            );
          })}
        </div>
...
export default function PromoPanelEditor({ panelId = null, onSuccess, onCancel }) {
  ...
  useEffect(() => {
    if (!isEdit) return;
    setLoading(true);
    fetch(`${API}/api/admin/promo-panels/${panelId}`, { credentials: 'include' })
      .then(r => r.json())
      .then(b => {
        const s = b.panel || {};
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-2xl shadow space-y-5">
      <h2 className="text-2xl font-bold text-gray-800">{isEdit ? 'Edit Promo Panel' : 'New Promo Panel'}</h2>

      {/* Image upload */}
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-2">Panel Image</label>
        <div
        ...
        >
          {image.url ? (
            ...
            <img src={image.url} alt="Promo panel preview" className="w-full h-full object-contain bg-gray-50" />
          ) : (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2">
              ...
              <span className="text-xs text-gray-300">Recommended: 256 × 256 px</span>
            </div>
          )}
          {uploading && (
      ...
      {/* Text fields */}
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Small text (above heading)</label>
      ...
      <div className="border-t pt-4">
        <ProductSearchSelect
          ...
          API={API}
        />
        <p className="text-xs text-gray-400 mt-1">Selected products will appear in the Popular Picks carousel on the homepage.</p>
      </div>
