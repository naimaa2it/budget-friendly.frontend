...
export default function MostPopular() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API}/api/analytics/most-popular?limit=${limit}`, {
        credentials: 'include',
      });
  ...
  const handleDelete = async (item) => {
    if (!confirm(`Reset view count for "${item.title}"? This cannot be undone.`)) return;
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Products ranked by total page views from all visitors (guests &amp; logged-in users).</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.map((item, idx) => {
                const thumb = item.images?.[0]?.url || '/assets/placeholder.svg';
                  ...
                  <tr key={item._id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-2.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-medium text-gray-800 truncate max-w-50">{item.title}</span>
                      </div>
                    </td>
