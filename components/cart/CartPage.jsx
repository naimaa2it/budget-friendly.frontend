...
import { useCart, getItemPrice, getItemCompareAtPrice } from '@/components/context/CartContext';
...
export default function CartPage() {
  ...
  const fbtProducts = React.useMemo(() => {
    const inCartIds = new Set(cartItems.map(i => String(i.product?._id || i.product?.id)));
  ...
  const originalTotal = cartItems.reduce(
    (sum, item) => sum + (getItemCompareAtPrice(item) || getItemPrice(item)) * item.quantity, 0
  );
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/products?badge=popular_pics&limit=12`)
      ...
      .then(json => {
        ...
        if (items.length > 0) { setRecommendedProducts(items); return; }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-3 md:px-4">
        ...
        {cartItems.map((item) => {
            const { product, quantity, cartKey, selectedColor, selectedSize } = item;
            ...
            const hasVariants = allColors.length > 0 || allSizes.length > 0 || product.variants?.length > 0;
            
            // Find color hex for the selected color
            const selectedColorObj = selectedColor ? allColors.find(c => c.name?.toLowerCase() === selectedColor?.toLowerCase()) : null;
            const colorHex = selectedColorObj?.hex || null;

            return (
              <div key={cartKey} className="p-3 md:p-6 border-b last:border-b-0">
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 relative">
                  ...
                  <div className="flex-1">
                    <h3 className="text-base md:text-lg font-semibold mb-1 pr-10 md:pr-0">{title}</h3>
                    ...
                    {(selectedColor || selectedSize) ? (
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        {hasVariants && (
                          <button onClick={() => { setEditItem(item); setEditMode('edit'); }} title="Edit variant"
                      ...
                      <button onClick={() => { setEditItem(item); setEditMode('edit'); }}
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-base md:text-lg font-bold text-red-600">৳{price.toFixed(2)}</span>
                        {compareAt > price && (
                    ...
                    {product.freeShipping && (
                      <p className="text-sm font-semibold text-green-700 mb-3">🚚 Free Shipping</p>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-4 md:gap-6">
                      <div className="flex items-center gap-3 md:gap-4">
                        <button
                          onClick={() => updateQty(cartKey, Math.max(1, quantity - 1))}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg md:text-xl font-bold text-gray-900">৳{(price * quantity).toFixed(2)}</span>
                      </div>
                      {itemSaved > 0 && (
                        <div className="text-sm text-green-600">
                          Saved <span className="font-semibold">৳{itemSaved.toFixed(2)}</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {fbtProducts.length > 0 && (
          <div className="my-8">
            <div className="flex items-center gap-3 mb-1">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Frequently Bought Together</h2>
            </div>
            <p className="text-gray-500 text-sm mb-4 ml-9">Customers who bought items in your cart also bought these</p>
        ...
        <div className="my-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800">Picked For You</h2>
          </div>
          ...
          {recLoading ? (
            ...
            <p className="text-gray-400 text-sm text-center py-6">No recommendations available right now.</p>
          )}
        </div>
      </div>
      {editItem && (
        <VariantEditModal
          item={editItem}
          mode={editMode}
          onClose={() => { setEditItem(null); setEditMode('edit'); }}
          onSave={(c, s, v, q) => {
