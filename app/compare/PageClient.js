...
const ROWS = [
  ...
  { key: "compareAtPrice", label: "Original Price", render: (p) => p.compareAtPrice ? `৳${Number(p.compareAtPrice).toLocaleString()}` : "—" },
  { key: "availability", label: "Availability", render: (p) => {
    const map = { in_stock: "In Stock", out_of_stock: "Out of Stock", pre_order: "Pre-Order" };
    return map[p.availability] || p.availability || "—";
  }},
  { key: "averageRating", label: "Rating", render: (p) => p.averageRating ? `${Number(p.averageRating).toFixed(1)} ★ (${p.reviewCount || 0})` : "No reviews" },
  { key: "sku", label: "SKU", render: (p) => p.sku || "—" },
  { key: "freeShipping", label: "Free Shipping", render: (p) => p.freeShipping ? "✓ Yes" : "✗ No" },
  { key: "rewardPoints", label: "Reward Points", render: (p) => p.rewardPoints ? `${p.rewardPoints} pts` : "—" },
  { key: "category", label: "Category", render: (p) => (typeof p.category === "object" ? p.category?.name : p.category) || "—" },
  { key: "warranty", label: "Warranty", render: (p) => p.warranty?.period || "—" },
];
  ...
  if (compareList.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-24 text-center">
        ...
        <Link href="/" className="inline-block bg-gray-900 text-white px-6 py-3 rounded-lg font-semibold hover:bg-gray-700 transition">
          Browse Products
        </Link>
      </div>
    );
        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <thead>
            <tr className="border-b border-gray-200">
              ...
              {compareList.map((p) => {
                ...
                const isOutOfStock = p.availability === "out_of_stock" || p.inventory === 0;
                return (
                  <th key={p._id} className="p-4 bg-white border-l border-gray-100 align-top min-w-[180px]">
          ...
          <tbody>
            {ROWS.map((row, rowIdx) => {
              ...
              const isBest = (val) => {
                if (row.key === "price") {
                  const nums = values.map((v) => parseFloat(v.replace(/[^\d.]/g, "")) || Infinity);
                  return parseFloat(val.replace(/[^\d.]/g, "")) === Math.min(...nums);
                }
                if (row.key === "averageRating") {
                  const nums = values.map((v) => parseFloat(v) || 0);
                  return parseFloat(val) === Math.max(...nums) && parseFloat(val) > 0;
                }
                return false;
              };

              return (
                <tr key={row.key} className={`border-b border-gray-100 ${rowIdx % 2 === 0 ? "bg-gray-50/50" : "bg-white"}`}>
                    ...
                    return (
                      <td
                        ...
                        className={`p-4 text-sm border-l border-gray-100 text-center ${
                          best ? "font-semibold text-green-700 bg-green-50/50" : "text-gray-700"
                        }`}
                      >
                        {val}
                        {best && <span className="ml-1 text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-bold">Best</span>}
                      </td>
                    );
                  })}
                </tr>
            ...
            {(() => {
              ...
              return [...allKeys].slice(0, 10).map((key, i) => (
                <tr key={`spec-${key}`} className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}`}>
                  <td className="p-4 font-medium text-gray-600 text-xs uppercase tracking-wide bg-gray-50 border-r border-gray-100">
                    {key}
                  </td>
                  {compareList.map((p) => {
                    const spec = (p.specifications || []).find((s) => s.key === key && s.type !== "header");
                    return (
                      <td key={p._id} className="p-4 text-sm border-l border-gray-100 text-center text-gray-700">
                        {spec?.value || "—"}
                      </td>
                    );
                  })}
