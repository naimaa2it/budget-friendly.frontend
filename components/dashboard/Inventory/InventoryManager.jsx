...
function StockBadge({ stock, threshold = 5 }) {
  if (stock <= 0)
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700"><span className="w-1.5 h-1.5 rounded-full bg-red-500 inline-block" />Out of Stock</span>;
  if (stock <= threshold)
    return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-700"><span className="w-1.5 h-1.5 rounded-full bg-amber-500 inline-block" />Low Stock</span>;
  return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700"><span className="w-1.5 h-1.5 rounded-full bg-green-500 inline-block" />In Stock</span>;
}

// ─── Stock bar ────────────────────────────────────────────────────────────────
function StockBar({ stock, threshold = 5, max = 100 }) {
  const pct = Math.min(100, (stock / Math.max(max, 1)) * 100);
  const color = stock <= 0 ? "bg-red-400" : stock <= threshold ? "bg-amber-400" : "bg-green-400";
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
        <div className={`h-full rounded-full transition-all ${color}`} style={{ width: `${pct}%` }} />
      </div>
      <span className="text-xs text-gray-700 font-semibold w-8 text-right">{stock}</span>
    </div>
  );
}
      ...
      {product.variants.map((v, i) => (
        <div key={i} className="flex items-center gap-3 text-xs text-gray-600">
          <span className="shrink-0 font-medium min-w-[6rem] truncate">
            {[v.color?.name, v.size].filter(Boolean).join(" / ") || v.name || `Variant ${i + 1}`}
          </span>
          ...
          <div className="flex-1 flex items-center gap-2">
            ...
            />
            <StockBadge stock={Number(v.inventory) || 0} threshold={product.lowStockThreshold || 5} />
          </div>
        </div>
      ))}
    </div>
...
export default function InventoryManager() {
  ...
  const [summary, setSummary] = useState({ total: 0, out_of_stock: 0, low_stock: 0, in_stock: 0 });
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      ...
      const r = await fetch(`${API}/api/admin/inventory?${params}`, { credentials: "include" });
      const d = await r.json();
      if (r.ok) {
        setItems(d.items || []);
        setSummary(d.summary || { total: 0, out_of_stock: 0, low_stock: 0, in_stock: 0 });
      ...
      if (r.ok) {
        ...
        setItems((prev) => prev.map((it) => {
          ...
          if (variantIndex != null) {
            const newVariants = it.variants.map((v, i) => i === variantIndex ? { ...v, inventory: newStock } : v);
            const totalStock = newVariants.reduce((s, v) => s + (Number(v.inventory) || 0), 0);
            ...
            return { ...it, inventory: newStock, totalStock: newStock, stockStatus };
          }
        }));
      }
    } catch {}
    finally { setSaving((s) => { const n = { ...s }; delete n[productId + (variantIndex ?? "")]; return n; }); }
  };
  ...
  const updateSettings = async (productId, field, value) => {
    try {
      ...
      setItems((prev) => prev.map((it) => it._id === productId ? { ...it, [field]: value } : it));
    } catch {}
  };
  ...
  const handleBulkSave = async () => {
    ...
    try {
      const updates = Object.entries(bulkPending).map(([key, val]) => {
        ...
        return { id, inventory: val, ...(vidx !== "main" ? { variantIndex: Number(vidx) } : {}) };
      });
  ...
  const FILTER_TABS = [
    { key: "all", label: "All Products", count: summary.total, color: "bg-gray-100 text-gray-700" },
    { key: "out_of_stock", label: "Out of Stock", count: summary.out_of_stock, color: "bg-red-100 text-red-700" },
    { key: "low_stock", label: "Low Stock", count: summary.low_stock, color: "bg-amber-100 text-amber-700" },
    { key: "in_stock", label: "In Stock", count: summary.in_stock, color: "bg-green-100 text-green-700" },
  ];
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {FILTER_TABS.map((f) => (
          ...
          >
            ...
            <div className={`inline-block px-2 py-0.5 rounded-full text-lg font-bold ${f.color}`}>{f.count}</div>
          </button>
        ))}
      </div>
      ...
      {Object.keys(bulkPending).length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
          <p className="text-sm text-amber-800 font-medium">{Object.keys(bulkPending).length} unsaved change{Object.keys(bulkPending).length > 1 ? "s" : ""}</p>
          <div className="flex gap-2">
            <button onClick={() => setBulkPending({})} className="text-xs text-gray-500 hover:text-gray-700">Discard</button>
            ...
            >{bulkSaving ? "Saving…" : "Save All"}</button>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border shadow-sm">
        ...
        <div className="px-5 py-4 border-b flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          ...
          <span className="text-xs text-gray-400 sm:ml-auto">{total} product{total !== 1 ? "s" : ""}</span>
        </div>
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-100">
            {items.map((product) => {
              ...
              return (
                <div key={product._id} className="px-5 py-3 hover:bg-gray-50 transition">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="shrink-0 w-10 h-10 rounded-lg bg-gray-100 overflow-hidden">
                      {thumb ? <img src={thumb} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300 text-lg">📦</div>}
                    </div>
                    ...
                    <div className="w-36 hidden sm:block">
                      <StockBar stock={product.totalStock} threshold={threshold} max={maxStock} />
                    </div>

                    {/* Badge */}
                    <div className="shrink-0 w-28 flex justify-end">
                      <StockBadge stock={product.totalStock} threshold={threshold} />
                    </div>
                    ...
                    <div className="shrink-0 flex items-center gap-1.5" title={product.allowOverselling ? "Overselling allowed — can sell when out of stock" : "Overselling off — cannot sell when out of stock"}>
                      <span className="text-xs text-gray-400 hidden lg:inline">{product.allowOverselling ? "Oversell ON" : "Oversell OFF"}</span>
                      <button
                        onClick={() => updateSettings(product._id, "allowOverselling", !product.allowOverselling)}
                  ...
                  {isExpanded && (
                    <div className="mt-2 pl-8 flex items-center gap-3 text-xs text-gray-500">
                      ...
                      <InlineStockEditor
                        ...
                        onSave={(val) => updateSettings(product._id, "lowStockThreshold", val)}
                      />
                      <span className="text-gray-400">(warn below this qty)</span>
                    </div>
                  )}
                </div>
              );
      ...
      <div className="bg-white rounded-xl border px-5 py-4 shadow-sm">
        ...
        <div className="grid sm:grid-cols-3 gap-3 text-xs text-gray-600">
          <div className="flex gap-2"><span className="text-green-500">✓</span><span><strong>Click any stock number</strong> to edit it inline. Press Enter or click away to save.</span></div>
          <div className="flex gap-2"><span className="text-amber-500">↕</span><span><strong>Expand variants</strong> (▶ arrow) to see and edit each variant's stock separately.</span></div>
          <div className="flex gap-2"><span className="text-blue-500">⟳</span><span><strong>Oversell toggle</strong> — when ON, the product can be ordered even if stock is 0.</span></div>
        </div>
      </div>
    </div>
  );
