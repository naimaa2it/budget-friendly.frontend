...
export default function AddToCartSection({ product, selectedColor = null, selectedSize = null }) {
  ...
  const handleNotify = async (e) => {
    e.preventDefault();
    if (!notifyValue.trim()) { setNotifyError('Please enter a value.'); return; }
    setNotifyLoading(true); setNotifyError('');
    try {
      const API = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
      const body = { productId: product._id, productTitle: product.title || product.name };
      if (notifyType === 'email') body.email = notifyValue.trim();
      else body.phone = notifyValue.trim();
      const res = await fetch(`${API}/api/waitlist`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
          ...
          <div className="rounded-2xl bg-[#e8f5f0] border border-[#c0e4d8] px-5 py-3 mt-1 max-w-sm">
            {notifySuccess ? (
              <div className="text-center py-2">
                ...
                <p className="text-sm text-green-700 mt-1">We&apos;ll notify you when it&apos;s back in stock.</p>
              </div>
            ) : (
              <>
                <p className="font-bold text-gray-800 text-base mb-0.5">In stock very soon!</p>
                <p className="text-sm text-gray-500 mb-3">Get notification in sec upon arrival</p>
                <div className="flex items-center gap-5 mb-3">
                  <label className="flex items-center gap-1.5 cursor-pointer text-sm font-medium text-gray-700">
                    <input type="radio" name="notifyType" value="email" checked={notifyType === 'email'} onChange={() => { setNotifyType('email'); setNotifyValue(''); }} className="accent-teal-600" />
                    Mail address
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer text-sm font-medium text-gray-700">
                    <input type="radio" name="notifyType" value="phone" checked={notifyType === 'phone'} onChange={() => { setNotifyType('phone'); setNotifyValue(''); }} className="accent-teal-600" />
                    Phone number
                  </label>
                </div>
                <form onSubmit={handleNotify} className="flex items-center gap-2">
                  <input
                    ...
                    placeholder={notifyType === 'email' ? 'ex. example@mail.com' : '+880 1XXX-XXXXXX'}
                ...
                {notifyError && <p className="text-red-600 text-xs mt-2">{notifyError}</p>}
              </>
            )}
          </div>
        </>
