...
export default function FollowUpManager() {
  ...
  const loadPeople = useCallback(async () => {
    const r = await fetch(`${API}/api/admin/admins`, { credentials: "include" });
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/admin/orders/${assignOrderId}`, { credentials: "include" })
  ...
  const assignToOrder = async (adminId) => {
    ...
    setAssigning(true);
    try {
      const r = await fetch(`${API}/api/admin/orders/${assignOrderId}/follow-up`, {
        method: "PUT",
        credentials: "include",
      ...
      {assignOrder && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
          ...
          <p className="text-sm text-amber-800 mt-1">
            {assignOrder.billingDetails?.name} · {assignOrder.billingDetails?.phone}
          </p>
            ...
            {!people.length && (
              <tr>
                <td colSpan={4} className="px-4 py-10 text-center text-gray-400">
                  ...
                  <Link href="/dashboard/authorized" className="text-rose-600 hover:underline">
                    Add one
                  </Link>
                </td>
              </tr>
