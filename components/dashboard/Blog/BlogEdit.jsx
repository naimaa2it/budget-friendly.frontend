...
export default function BlogEdit({ postId }) {
  ...
  const handleLink = () => {
    ...
    if (node && node.tagName === 'A') {
      ...
      } else if (container.parentElement && container.parentElement.tagName === 'A') {
    ...
    if (linkElement) {
      ...
      const action = prompt(`Current link: ${currentUrl}\n\nOptions:\n1. Enter a NEW URL to change the link\n2. Type "remove" to delete the link\n3. Click Cancel to keep as is`, currentUrl);
  ...
  const insertImageFile = async (file) => {
    ...
    fd.append('file', file);
    fd.append('folder', 'yourhaat/blog/images');
    ...
    fetch(`${API}/api/admin/blog/${postId}`, { credentials: 'include' })
      ...
      .then(b => {
        if (b.post) {
          ...
          if (b.post.featuredImage && typeof b.post.featuredImage === 'object') {
            setFeaturedImage(b.post.featuredImage);
          } else if (b.post.featuredImage && typeof b.post.featuredImage === 'string') {
          ...
          setPublishDate(b.post.publishDate ? new Date(b.post.publishDate).toISOString().split('T')[0] : '');
    ...
    <div className="bg-white p-6 rounded shadow max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-6">
        <div className="flex-1 space-y-4 min-w-0">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Excerpt(Optional need for featured blog)</label>
            <textarea 
      ...
      <div className="mb-6 p-4 border rounded-lg bg-gray-50">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">Leave empty to use creation date</p>
          </div>
      ...
      <div className="mb-6 p-4 border rounded-lg bg-gray-50">
        <h3 className="text-sm font-semibold mb-3">Dynamic Sections (FAQ, Accordion, Steps)</h3>
