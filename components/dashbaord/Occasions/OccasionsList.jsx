...
export default function OccasionsList() {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Delete this occasion section? This cannot be undone.')) return;
    setDeleting(id);
    try {
  ...
  const handleMoveOrder = async (idx, dir) => {
    ...
    try {
      await fetch(`${API}/api/admin/occasions-reorder`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify(updated.map(s => ({ _id: s._id, order: s.order }))),
      });
    } catch {
  ...
  if (view === 'edit' && editId) return (
    <OccasionEditor
      sectionId={editId}
      onSuccess={() => { load(); setView('list'); setEditId(null); }}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Manage festival / occasion mini-banner sections shown on the homepage.
          </p>
        </div>
        <button
            ...
            >
              ...
              <div className="flex gap-1 flex-shrink-0">
                {section.cards?.slice(0, 4).map((card, ci) => (
                  <div key={ci} className="w-10 h-8 rounded overflow-hidden bg-gray-100 border border-gray-200">
                    {card.image?.url
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-800 truncate">{section.title}</p>
                <p className="text-xs text-gray-400 mt-0.5">
                  {section.cards?.length || 0} card{section.cards?.length !== 1 ? 's' : ''} · View All → {section.viewAllLink}
                </p>
              </div>
              ...
              <div
                ...
                className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors flex-shrink-0 ${section.isActive ? 'bg-green-500' : 'bg-gray-300'}`}
              >
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${section.isActive ? 'translate-x-5' : 'translate-x-0'}`} />
              </div>

              {/* Edit / Delete */}
              <div className="flex gap-2 flex-shrink-0">
                <button
                  onClick={() => { setEditId(section._id); setView('edit'); }}
