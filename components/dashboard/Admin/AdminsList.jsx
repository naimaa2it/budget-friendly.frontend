...
export default function AdminsList() {
  ...
  const handleDeactivate = async (id) => {
    ...
    try {
      const resp = await fetch(`${API}/api/admin/admins/${id}/deactivate`, { method: 'PUT', credentials: 'include' });
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-4 sm:p-6 rounded shadow">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        <a href="/dashboard/authorized/new" className="px-3 py-2 bg-green-600 text-white rounded text-sm text-center shrink-0">Create admin/moderator</a>
      </div>
              ...
              <tr key={a._id} className="border-t">
                ...
                <td className="py-3">
                  <div className="flex gap-2">
                    <a className="px-2 py-1 border rounded text-sm" href={`/dashboard/authorized/${a._id}/profile`}>Profile</a>
                    <a className="px-2 py-1 border rounded text-sm" href={`/dashboard/authorized/${a._id}`}>Edit</a>
                    ...
                    {user?.role === 'admin' && (
                      <button className="px-2 py-1 border rounded text-sm text-red-600" onClick={() => handleDelete(a._id)}>Delete</button>
                    )}
                  </div>
                </td>
              </tr>
