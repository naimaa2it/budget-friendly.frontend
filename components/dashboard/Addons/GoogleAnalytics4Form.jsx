...
export default function GoogleAnalytics4Form() {
  ...
  const [cfg, setCfg] = useState({ measurementId: "", active: false, installed: true });
    ...
    fetch(`${API}/api/admin/settings`, { credentials: "include" })
      ...
      .then((b) => {
        if (b.settings?.googleAnalytics4) {
          setCfg((prev) => ({ ...prev, ...b.settings.googleAnalytics4, installed: true }));
        }
      })
      .catch(console.error)
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-gray-900">Edit Google Analytics 4</h1>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">Measurement ID</label>
          <input
            type="text"
          ...
          <p className="mt-1.5 text-xs text-gray-400">
            Find in Google Analytics → Admin → Data Streams → your stream → Measurement ID.
          </p>
        </div>

        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-amber-900 text-xs">
          <p className="font-semibold mb-1">How to get your GA4 Measurement ID:</p>
          <ol className="list-decimal pl-4 space-y-1">
            <li>Go to <strong>analytics.google.com</strong> and sign in</li>
            <li>Click Admin (gear icon) → Data Streams</li>
            <li>Select your web stream</li>
            <li>Copy the <strong>Measurement ID</strong> — format: <code className="bg-amber-100 px-1 rounded">G-XXXXXXXXXX</code></li>
          </ol>
        </div>
