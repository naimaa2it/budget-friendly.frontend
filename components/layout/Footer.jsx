              ...
              <ul className="mt-4 space-y-1.5 text-sm text-[#202020]">
                {footerInfo?.phone && (
                  <li className="flex items-center gap-2">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.07 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 2.92 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                    <a href={`tel:${footerInfo.phone}`} className="hover:text-red-600">{footerInfo.phone}</a>
                  </li>
                )}
                {footerInfo?.email && (
                  <li className="flex items-center gap-2">
                    ...
                    <a href={`mailto:${footerInfo.email}`} className="hover:text-red-600">{footerInfo.email}</a>
                  </li>
                )}
                {footerInfo?.address && (
                  <li className="flex items-start gap-2">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="mt-0.5 shrink-0"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
              ...
              <div className="flex items-center gap-3 mt-4">
                {socialLinks?.facebook?.enabled !== false && socialLinks?.facebook?.url && (
                  <a href={socialLinks.facebook.url} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
                    style={{ color: '#1877F2' }} className="transition-opacity hover:opacity-80">
                ...
                {socialLinks?.instagram?.enabled !== false && socialLinks?.instagram?.url && (
                  <a href={socialLinks.instagram.url} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                    style={{ color: '#E1306C' }} className="transition-opacity hover:opacity-80">
                ...
                {socialLinks?.twitter?.enabled !== false && socialLinks?.twitter?.url && (
                  <a href={socialLinks.twitter.url} target="_blank" rel="noopener noreferrer" aria-label="Twitter / X"
                    style={{ color: '#000000' }} className="transition-opacity hover:opacity-70">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                ...
                {socialLinks?.tiktok?.enabled !== false && socialLinks?.tiktok?.url && (
                  <a href={socialLinks.tiktok.url} target="_blank" rel="noopener noreferrer" aria-label="TikTok"
                    style={{ color: '#010101' }} className="transition-opacity hover:opacity-70">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                ...
                {socialLinks?.youtube?.enabled !== false && socialLinks?.youtube?.url && (
                  <a href={socialLinks.youtube.url} target="_blank" rel="noopener noreferrer" aria-label="YouTube"
                    style={{ color: '#FF0000' }} className="transition-opacity hover:opacity-80">
