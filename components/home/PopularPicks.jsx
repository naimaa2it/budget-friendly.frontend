...
export default function PopularPicks() {
    ...
    const fetchPopular = async () => {
      ...
      try {
        ...
        try {
          ...
          const firstActive = (panelJson.items || []).find(p => p.isActive && Array.isArray(p.productIds) && p.productIds.length > 0);
          if (firstActive) {
  ...
  const activePromoPanel = promoPanels.length > 0 ? promoPanels[currentSlide % promoPanels.length] : null;
      ...
      <div className="relative flex gap-6 items-stretch">
        ...
        <div className="hidden lg:block w-64 self-stretch">
          <div className="relative h-full rounded-lg overflow-hidden">
            <Image
              src={activePromoPanel.image?.url || '/assets/placeholder.svg'}
            ...
            <div className="absolute inset-0 z-10 p-6 text-center flex flex-col justify-center">
              ...
              {activePromoPanel.title && (
                <h2 className="text-2xl font-bold text-orange-600 mb-4">{activePromoPanel.title}</h2>
              )}
              {activePromoPanel.buttonText && activePromoPanel.buttonLink && (
                <a href={activePromoPanel.buttonLink}
          ...
          >
            {loading ? (
              Array(slidesToShow).fill(0).map((_, i) => (
                <div key={i} className="bg-white border border-[#F1E4D8] rounded-xl shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-300 cursor-pointer h-full">
