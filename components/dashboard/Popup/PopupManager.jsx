...
export default function PopupManager() {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      fd.append('file', file);
      fd.append('folder', 'SmartBuyBD/popups');
  ...
  const handleSave = async () => {
    if (!image.url) { alert('Please upload a popup image first.'); return; }
    setSaving(true);
    try {
  ...
  const handleDelete = async () => {
    if (!confirm('Remove the popup image and disable popup? This cannot be undone.')) return;
    try {
      const resp = await fetch(`${API}/api/admin/popup`, { method: 'DELETE', credentials: 'include' });
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          Shown to visitors after 3 seconds. Auto-closes after 6 seconds if not dismissed.
        </p>
      </div>
          ...
          {image.url ? (
            <div className="relative w-full" style={{ minHeight: '220px' }}>
              ...
              {!uploading && (
                <button
                  type="button"
                  onClick={e => { e.stopPropagation(); setImage({ url: '', public_id: '' }); }}
            ...
            <div className="flex flex-col items-center justify-center h-full py-12 gap-2 text-gray-400">
              <svg className="w-10 h-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 15l5-5 4 4 3-3 4 4"/><circle cx="8.5" cy="8.5" r="1.5"/></svg>
              <span className="text-sm">Click or drag & drop to upload image</span>
            </div>
          )}
        </div>
      ...
      {/* Link */}
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Click-through Link</label>
        <input
          type="text"
          value={link}
        ...
        />
        <p className="text-xs text-gray-400 mt-1">Where visitors go when they click the popup image.</p>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-700 font-medium">{isActive ? 'Popup enabled' : 'Popup disabled'}</span>
      </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        {saved && <span className="self-center text-sm text-green-600 font-medium">✓ Saved!</span>}
      </div>
