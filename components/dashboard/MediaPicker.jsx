...
export default function MediaPicker({ open, onSelect, onClose }) {
  ...
  const load = useCallback(async (reset = true) => {
    ...
    try {
      ...
      setItems(prev => reset ? (b.items || []) : [...prev, ...(b.items || [])]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
                ...
                return (
                  <button
                    ...
                    className={`relative aspect-square rounded-xl overflow-hidden border-2 transition ${
                      isSel ? 'border-blue-500 ring-2 ring-blue-400' : 'border-transparent hover:border-gray-300'
                    }`}
                  >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t bg-gray-50 rounded-b-2xl">
          <p className="text-sm text-gray-500">
            {selected
              ? <span className="text-gray-700 font-medium truncate max-w-xs inline-block">{selected.public_id}</span>
